...
기존 `agent-harness-baseline` 는 두 맥북의 데이터(스킬, 규칙, 메모리)를 동기화한다. 이 spec은 그 위에 **체험 레이어**를 얹는다. 사용자가 "두 맥북이 진짜로 살아있는 한 시스템"으로 느끼게 한다.
...
`~/.config/agent-harness-baseline/.machine.json` 에 머신별 저장. hostname 기준 자동 매칭 + 수동 override 가능.
...
- 위치: `agent-harness-baseline/state/activity/{persona}.jsonl` (git 추적, 양 머신 동기화)
- 형식: JSONL — 한 줄 = 한 이벤트
- 영구 보존 (사용자 결정)
...
- 새 launchd: `launchd/com.denny.agent-harness-baseline-digest.plist` (매일 08:00)
...
- **Codex 한계** — 메모리 누적 / 훅 X. 메모리는 CC → Codex 단방향 (agent-harness-baseline 한계 그대로).
...
- `launchd/com.denny.agent-harness-baseline-digest.plist` (C3 일일요약)
