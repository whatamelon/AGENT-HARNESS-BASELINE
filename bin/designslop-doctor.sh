...
if [ -f "$HOOKS/designslop_detectors.py" ] && \
  ...
  bad "designslop_detectors.py 누락/문법오류" "claude-sync git pull && bash $SSOT/bin/install.sh"; fi
...
for f in no-design-slop.md no-decorative-eyebrow.md designslop-rubric.json; do
  [ -f "$RULES/$f" ] && ok "rule: $f" || bad "rule 누락: $f" "claude-sync git pull"
done
...
for am in "$HOME/.claude/CLAUDE.md" "$HOME/AGENTS.md" "$HOME/.codex/AGENTS.md"; do
  [ -f "$am" ] && grep -qiE "design.?slop|no-design-slop|claude-sync/claude/rules" "$am" 2>/dev/null \
