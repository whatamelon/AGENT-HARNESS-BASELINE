...
# Two-tier skill layout: skills.full = source content store (real files);
# codex/skills/<name> = symlink into it (+ generated wrappers / .system as real dirs).
SKILLS_FULL="${SKILLS_FULL:-$SSOT/codex/skills.full}"
...
# Rebuild the codex/skills symlink view from the canonical skills.full content
# store: for every source skill in skills.full lacking a codex/skills entry,
# create codex/skills/<name> -> ../skills.full/<name>. Existing symlinks,
# generated command wrappers (real dirs), and .system are left untouched.
rebuild_skill_symlinks_from_full() {
  [[ -d "$SKILLS_FULL" ]] || return 0
  local entry name link
  for entry in "$SKILLS_FULL"/*; do
    [[ -d "$entry" ]] || continue
    name="$(basename "$entry")"
    link="$SSOT/codex/skills/$name"
    [[ -e "$link" || -L "$link" ]] && continue   # already present (symlink / wrapper / override)
    ln -s "../skills.full/$name" "$link"
    log "Linked codex/skills/$name -> ../skills.full/$name"
  done
}
...
ensure_codex_links() {
  ...
  # Skills are two-tier. When skills.full exists it is the canonical content
  # store — rebuild the codex/skills symlink view from it instead of rsync'ing
  # ~/.codex/skills over it (which would re-materialize real files and clobber
  # the symlink layout). Falls back to the legacy copy only on a pre-migration
  # tree with no skills.full.
...
sync_commands_to_skills() {
  ...
  for cmd in "$commands_dir"/*.md; do
    ...
    # If a source skill of the same name exists, $target resolves (possibly via
    # the codex/skills -> skills.full symlink) to a non-generated SKILL.md and
    # we skip — generated command wrappers never clobber a source skill. Only
    # names with no source skill get a fresh AUTO-GENERATED wrapper (real dir).
