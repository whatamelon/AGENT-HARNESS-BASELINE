...
must_not:
  ...
  - monochrome_status_filter                   # status 칩에 의미색 신호가 0이면 위반. active 배경=흑 강조는 허용하되 컬러 dot(또는 tint)로 상태색 유지 (하이브리드, 2026-05-31)
  # 강화 (2026-05-29 audit)
...
verifier_probes:
  ...
  - id: sticky-table-header
    layer: L2
    rule: "DataTable thead className 에 'sticky top-0' + opaque bg (bg-muted/bg-card/bg-background, 반투명 /NN suffix 금지). wrapper 에 overflow-hidden 으로 sticky 무력화 금지. **주의(2026-05-31): className 에 bg-muted 있어도 런타임 computed bg 가 transparent 면 행이 비쳐 sticky 가 깨져 보인다 — @theme 토큰 hsl() 미래핑이 원인(§04 §4.7, probe surface-bg-opaque-runtime). position 만 의심 말고 computed bg 부터 확인.**"
  - id: status-filter-colored
    layer: L2
    rule: "FilterChip 은 tone(StatusTone) 지원 + paramName==='status' 일 때 toneFor(value) 자동 색칠. idle=컬러 dot+tint. active=강조 배경(bg-foreground/흑 허용) + 컬러 dot 유지(상태색 신호 보존). 의미색 신호가 전혀 없는 순수 흑백 토글만이면 위반"
...
- idle = 옅은 tint + 컬러 dot. **active = 강조 배경(`bg-foreground` 검정 + `text-background` 흰 텍스트) + 컬러 dot 유지** (하이브리드, 2026-05-31 사용자 결정). 진한 tint active 도 허용. 핵심: active 에서도 컬러 dot 으로 상태색 신호를 남긴다. tone 매핑은 `StatusBadge` 와 동일 의미체계(SSOT §17).
- 비-status enum(소스/유형 등)은 기존 무채 칩 유지 — 색은 상태 의미에만.

```tsx
const resolvedTone = tone ?? (paramName === 'status' ? toneFor(value) : undefined)
// active = bg-foreground text-background (강조) + CHIP_TONE[tone].dot 유지(상태색 신호).
// idle  = CHIP_TONE[tone].idle (tint + dot). resolvedTone 없으면(비-status) 흑백 토글.
```

## 12.16 Staged 필터 + 검색 버튼 (2026-05-31 사용자 결정)

기본 동작 = **staged**. 검색 input·FilterChip 토글은 즉시 URL 을 쓰지 않고 toolbar 내부 pending state 에만 누적한다. 운영자가 **[검색] 버튼**(또는 검색 input Enter)을 눌렀을 때 한 번에 commit → URL(`q` + 필터 배열) 갱신 → 서버 재조회. 이유: 필터 여러 개를 고르는 동안 매 토글마다 재조회·리렌더가 깜빡임·낭비를 만들었다.

- **다중 선택.** 같은 그룹(예: status)에서 여러 값 동시 선택 = URL 반복 파라미터(`status=active&status=hidden`). 서버 query 는 `.in(col, values)`. page.tsx 는 `searchParams[param]` 를 `string | string[]` 로 받아 배열 정규화(단일 string 도 `[string]`).
- **commit 만 URL 을 쓴다.** chip/search 는 pending 만 갱신. [검색]/Enter 가 `new URLSearchParams(sp.toString())` clone → 기존 필터 param 전부 delete 후 pending 배열 append + `q` set + `page` delete → `router.replace`. §12.12 param 보존 패턴과 동일.
- **[검색] + [초기화] 쌍.** active param 있으면 [초기화](전체 reset) 노출(§12.15). pending 이 commit 된 URL 과 다르면 [검색] 버튼 강조(dirty indicator) — 적용 안 된 변경이 보이게.
- **url-state-sync 충족.** commit 시점에 location.search 갱신 → back/forward 로 필터 set 복원. staged 라도 최종 state 는 URL 단일 진실(§14.4).
- rightSlot 의 context picker(scope 등)는 staged 대상 아님 — 즉시 적용 유지(필터가 아니라 뷰 전환).
