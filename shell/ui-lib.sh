...
# 인트로 워드블록 — "HELLO AIDP" (마법사 진입 시 맨 처음)
ui_hello_aidp() {
  echo
  printf "${UI_BOLD}${UI_ROSE}"
  cat <<'ART'
    █   █  █████  █      █      █████         █████  █████  ████   █████
    █   █  █      █      █      █   █         █   █    █    █   █  █   █
    █████  ████   █      █      █   █         █████    █    █   █  █████
...
ART
  printf "${UI_R}"
  printf "${UI_DIM}${UI_GRAY}      ░▒▓ 8-bit boot · agent-harness-baseline ▓▒░${UI_R}\n\n"
}

# 8비트 칩튠 인트로 BGM — 백그라운드 재생(비차단), 10초 클립.
# 게이팅: 비TTY / CI / AHB_NO_BGM / 도구 부재 → 무음. (auto/verify 등은 호출부에서 차단)
ui_chiptune_bgm() {
  [[ -t 1 ]] || return 0
  [[ -z "${CI:-}" ]] || return 0
  [[ -z "${AHB_NO_BGM:-}" ]] || return 0
  command -v python3 >/dev/null 2>&1 || return 0
  command -v afplay  >/dev/null 2>&1 || return 0
  local ssot="${SSOT:-$HOME/.config/agent-harness-baseline}"
  local gen="$ssot/shell/bgm-gen.py"
  [[ -f "$gen" ]] || return 0
  local wav="${TMPDIR:-/tmp}/ahb-hello-aidp-bgm.wav"
  # 캐시 없으면 10초 합성(≈0.3s). 실패해도 조용히 통과.
  [[ -s "$wav" ]] || python3 "$gen" "$wav" 10 >/dev/null 2>&1 || return 0
  # 완전 분리 백그라운드 재생 — 마법사를 막지 않음, 10초 클립이라 자체 종료.
  ( afplay "$wav" >/dev/null 2>&1 & ) >/dev/null 2>&1
  return 0
}
